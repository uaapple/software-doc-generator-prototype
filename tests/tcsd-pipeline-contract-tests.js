...
async function createHarness(mode = "passed") {
  ...
  const executor = async (index, _input, job) => {
    ...
    if (index === 8) { const sim = path.join(outputDir, "simulation.json"); await writeFile(sim, "{}"); raw.artifacts = [{ path: rel(root, workbook), kind: "xlsx", role: "workbook" }, { path: rel(root, sim), kind: "json", role: "simulation" }]; raw.evidence = { simulationResult: rel(root, sim), expValueCount: 1 }; }
