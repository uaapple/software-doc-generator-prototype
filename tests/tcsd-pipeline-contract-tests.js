...
async function createHarness(mode = "passed") {
  ...
  const executor = async (index, _input, job) => {
    ...
    if (index === 8) { const sim = path.join(outputDir, "simulation.json"); await writeFile(sim, "{}"); raw.artifacts = [{ path: rel(root, workbook), kind: "xlsx", role: "workbook" }, { path: rel(root, sim), kind: "json", role: "simulation" }]; raw.evidence = { simulationResult: rel(root, sim), expValueCount: 1, simulationValueCount:1, workbookBackfillCount:1, caseOutputCounts:{"3:TC_001":{Output:1}}, backfillItems:[{row:3,testId:"TC_001",step:1,output:"Output",value:1}] }; }
    ...
    if (index === 11) { if (mode !== "repair") { raw.status = "skipped"; raw.skipReason = mode === "passed" ? "首轮达标" : "修正未应用"; raw.artifacts = []; } else { const sim = path.join(outputDir,"final-simulation.json"); await writeFile(sim,"{}"); raw.artifacts.push({path:rel(root,workbook),kind:"xlsx",role:"workbook"},{path:rel(root,sim),kind:"json",role:"simulation"}); raw.evidence={simulationResult:rel(root,sim),expValueCount:1,simulationValueCount:1,workbookBackfillCount:1,caseOutputCounts:{"3:TC_001":{Output:1}},backfillItems:[{row:3,testId:"TC_001",step:1,output:"Output",value:1}]}; raw.coverage = coverage(85); } }
    if (index === 12) {
