...
- 文档状态：第一版可运行功能的架构基线
- 功能代码基线：`40ce4d236813201ad4776e1c3b2beec08d0f6f1e`
- 第 4 轮双端 HTTP 合成验收提交：`05081b45259c6e5a3209a4b753be352d35066cf2`
- 第 5 轮容器合成验收基线：`8c6d4ca87d48ae525781f7a8b53514110eacfa00`
...
当前代码已经具备九阶段目录、九个同名技能、Worker 作业服务、任务级 MATLAB 租约、平台任务接线、阶段展示和最终 DOCX 传输。本地 `linux/amd64` Platform、Worker 镜像已经构建通过，两个容器也已完成公开上传到最终 DOCX 下载的合成验收。这证明容器、HTTP、九阶段宿主编排、真实九技能镜像快照与哈希、Gateway HTTP 租约和制品链可以串联运行；但 Hermes 生成、Hermes 技能列表发现、Hermes 会话编号和 MATLAB 工具调用仍使用合成替身。截至本文档基线，还没有使用真实 Hermes 推理、真实 MATLAB/Simulink 和经授权的同类业务输入完成浏览器人工验收。因此，当前状态不能表述为“真实业务流程已经跑通”。
...
- 使用真实 Platform 与 Worker HTTP 应用、真实公开上传接口和真实九阶段作业服务完成的双任务合成全链测试；
- 九阶段合成功能轨迹，验证九个不同 Hermes 会话、一个任务级 MATLAB 会话、全部必需制品和最终 ZIP 型 DOCX。
- 本地构建通过的 `linux/amd64` Platform 和 Worker 镜像；
- 使用两个真实应用容器、真实 Platform/Worker HTTP、真实九阶段作业服务、镜像内真实九技能快照与内容哈希、真实 Gateway HTTP 租约完成的双任务容器合成全链测试；
- 从公开接口下载并通过 ZIP 完整性检查的有效 DOCX、双任务工作区与租约隔离，以及 Worker 上传会话清理。

以上证据只能证明合同、编排、镜像闭包和合成链路。容器合成验收中的 Hermes 生成、Hermes 技能列表发现、Hermes 会话编号和 MATLAB 工具调用是显式合成替身，不构成这些外部能力的真实验收。下列验收尚未完成：

- 九个阶段使用真实 Hermes 模型推理；
- Worker 中的真实 Hermes 完成九技能安装发现，并返回九个真实会话编号；
- Gateway 连接真实原生 MATLAB/Simulink，并在一个租约中完成阶段 1 至阶段 9；
- 使用经授权的真实或同类业务 `.slx`、`.mat`、可选 `.m` 和项目 addon；
- 从真实前端创建任务、持续查看九阶段、下载并人工复核最终 DOCX；
- 浏览器人工操作下的 Mac Platform 容器、Worker 容器、真实 Hermes 和宿主原生 MATLAB Gateway 组合验收。

## 第 4 轮已完成的双端 HTTP 合成验收

提交 `05081b45259c6e5a3209a4b753be352d35066cf2` 新增
`tests/software-detail-full-http-flow-tests.mjs`，并把它接入默认隔离测试套件。
该测试没有绕过 HTTP 去直接拼接平台结果，而是在临时目录内启动真实
Platform 应用和真实 Worker HTTP 应用，验证了以下链路：

1. 两个任务都通过平台公开的多部分上传接口提交合成 `.slx`、`.mat`、`.m`、
   项目编号和 Worker 选择，公开创建接口返回 HTTP 202 和包含九个等待阶段的任务。
2. Platform 真实使用上传模式把工作区发送到 Worker，Worker 真实接收多部分请求，
   再由 `SoftwareDetailPipelineJobService` 创建并持久化九阶段作业。
3. 每个 Worker 作业都按 100 至 900 的目录顺序调用九个阶段，阶段调用记录中的
   `skillName` 与九个目录技能逐项一致；每个任务记录九个互不相同的合成 Hermes
   会话编号，两个任务合计 18 个编号且互不重复。
4. 每个任务建立一个独立的合成 MATLAB 租约，阶段 2 至阶段 9 开始前共读取同一
   租约八次；阶段 9 的合成制品通过真实 Worker 宿主验证后，才关闭该任务租约。
5. 公开任务详情真实观察到九阶段中间状态；平台的有限轮询退出后，后台对账定时器
   继续查询既有 Worker 作业，并最终把两个公开任务都同步为完成。
6. Worker 终态查询真实生成 DOCX 传输信封；测试逐项核对 Base64 字节、文件大小
   和 SHA-256。Platform 使用正式传输客户端物化文件，公开下载接口返回与 Worker
   合成源字节完全一致的内容。
7. 两个任务拥有不同的平台任务编号、Worker 作业编号、任务工作区、Gateway
   workspace、租约、MATLAB 会话标识和 DOCX 字节；每个输出目录只包含本任务的
   一个最终 DOCX，证明这条合成链没有跨任务串线。
8. 两个终态 Worker 上传会话目录都被删除，持久化作业中的上传目录引用被清空并
   记录清理时间；平台任务和公开下载制品仍然保留。

这项验收中的产品调度、平台/Worker HTTP、九阶段作业服务、候选制品验证、传输、
物化、后台对账、下载和上传会话清理都使用真实实现。唯一替身是两类外部能力：

- Hermes 执行与推理使用记录型阶段执行器，技能发现快照也是合成数据；因此测试只
  证明九个技能名称和九个唯一测试会话被正确调度，不证明真实 Hermes 命令、
  斜杠技能调用、技能安装发现或模型推理已经成功。
- MATLAB 使用记录型租约客户端；因此测试只证明单租约生命周期与任务隔离，
  不证明真实 Gateway、原生 MATLAB/Simulink 或模型读取已经成功。

该新增全链测试没有单独覆盖非 JSON 候选、Worker 明确失败、临时断连、运行中删除
HTTP 409 和旧任务兼容；这些情况继续由已有 Worker、平台、传输和前端专项测试
覆盖。它也没有启动浏览器、真实 Hermes、真实 MATLAB，没有使用真实业务模型，
生成的是合成 DOCX 字节。因此，第 4 轮结论只能表述为“Platform 到 Worker 的双端
HTTP 合成功能链通过”，不能表述为实际 Hermes/MATLAB 或用户真实业务验收通过。

## 第 5 轮已完成的 Mac 双容器合成验收

第 5 轮分为两个层次，均只使用本地开发验收资源，不创建生产标签、不修改生产分支、不部署生产服务。

### A 层：本地镜像构建

本地 `linux/amd64` Platform 镜像和 Worker 镜像均已构建通过。构建所依据的产品运行代码提交为 `bb01d5036df7b79240825d6b32af7168975b78aa`，本地精确标签分别为 `sdg-platform:sdd-wave5-bb01d50` 和 `sdg-hermes-worker:sdd-wave5-bb01d50`。Platform 镜像闭包包含软件详设平台所需的阶段目录与作业合同；Worker 镜像包含九个阶段技能、共享运行资料、原始 DOCX 模板和生成 DOCX 所需依赖。

从 `bb01d5036df7b79240825d6b32af7168975b78aa` 到本轮验收基线 `8c6d4ca87d48ae525781f7a8b53514110eacfa00`，后续变更只新增或调整开发专用的容器合成测试、Compose 测试夹具和 `package.json` 测试命令，没有改变产品运行代码或镜像输入。因此，B 层复用上述两个已构建镜像，不代表拿旧产品代码替代当前源码。

### B 层：双容器合成功能链

双容器合成验收使用真实 Platform 容器、真实 Worker 容器、真实 Platform/Worker HTTP 接口和真实 `SoftwareDetailPipelineJobService`。它完成并核对了：

1. 两个任务从平台公开多部分上传接口创建，各自依次完成九阶段。
2. Worker 从镜像内九个真实技能目录和共享运行资料计算技能快照与 SHA-256 哈希；测试提供合成的 Hermes 技能列表响应来选择这份真实快照。
3. 每个任务记录九个互不重复的阶段会话编号；这些编号由合成阶段执行器提供，不是真实 Hermes 会话。
4. Worker 通过真实 Gateway HTTP 租约接口为每个任务建立、查询和关闭一个独立租约；Gateway 服务、认证、租约状态和关闭流程使用真实实现，租约底层原生 MATLAB 客户端是合成替身，测试没有执行 MATLAB 工具调用。
5. 第 9 阶段生成有效 ZIP 型 DOCX，平台经正式传输和物化链登记制品，公开下载接口返回可解包且包含合成验收标记的文档。
6. 两个任务的公开任务编号、Worker 作业编号、工作区、租约、会话编号和 DOCX 内容彼此隔离。
7. 终态 Worker 上传会话目录被清理，平台任务和下载制品仍保留。

因此，B 层结论是“软件详设双容器合成功能链通过”。它不证明真实 Hermes 生成质量、真实 `skills-list` 技能发现、真实 Hermes 会话、真实 MATLAB/Simulink 工具调用或真实业务内容质量。

容器合成测试的默认模式会从当前源码构建镜像，以保证源码与镜像一致。网络不可达而无法重新取得基础依赖时，可以显式选择预构建镜像模式；该模式要求分别提供已存在的 `linux/amd64` Platform 和 Worker 精确标签，拒绝 `latest`，并固定使用“不构建、不拉取”的启动方式。预构建模式只用于可追溯的本地重复验收，不能把来源不明或与产品代码不一致的镜像当作当前源码证据。

### 仍待用户完成的真实验收

1. 启动真实 Hermes 和宿主原生 MATLAB Gateway，确认 Worker 能发现九个真实技能并实际调用 MATLAB/Simulink。
2. 使用经授权的同类业务 `.slx`、`.mat`、可选 `.m` 和项目 addon，从 `/software-detail-design-generation` 页面人工上传、选择项目与 Worker 并发起任务。
3. 观察九个真实 Hermes 会话及同一任务 MATLAB 租约的完整生命周期，下载并人工复核最终 DOCX 的模板结构、原生列表、模块边界、输出覆盖和“设计依据”默认留空等原技能要求。
4. 核对宿主 addon 未被修改、任务间没有串线，并保存脱敏的阶段摘要、制品哈希和验收结论。

完成以上步骤前，只能报告“自动化与双容器合成验收通过”，不能报告“真实 Hermes、真实 MATLAB 或用户真实业务验收完成”。
