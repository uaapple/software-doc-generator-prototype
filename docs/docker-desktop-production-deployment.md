...
companion v5 是 Windows PowerShell 5.1 兼容发布：本地 evaluate token 使用
...
v5 ZIP 还包含独立、只读且由 manifest size/SHA-256 保护的
...
v5 在停服务前从未跟踪容器 env 的 `SDG_CONTAINER_DATA_DIR`、
...
生产顺序固定为：核对 v5 资产 → 从新临时目录运行受保护的 PS5.1 测试 →
