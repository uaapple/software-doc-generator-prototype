...
companion v8 继承 v7 的 Windows PowerShell 5.1 兼容边界：本地 evaluate token 使用
...
v8 ZIP 还包含独立、只读且由 manifest size/SHA-256 保护的
...
v8 在停服务前从未跟踪容器 env 的 `SDG_CONTAINER_DATA_DIR`、
...
Windows/Docker Desktop 合法绝对路径可使用 `C:\...` 或 `C:/...`；部署工具会
先规范化为本机 canonical Windows 路径再比较、验证和写回。`C:foo`、单斜杠
root-relative、POSIX/相对路径及盘符根仍 fail-closed。v4 因只接受反斜杠而
错误拒绝正式 env 示例，保持不可变且禁止继续部署。

新 Worker、重建节点或启用软件详设九阶段的现有 Worker，生产顺序固定为：
核对 v8 资产 → 从新临时目录运行
受保护的 PS5.1 测试 → `-ProvisionDirectories` → `-ValidateOnly` →
管理员 PowerShell 正式执行 → 验证 `/health`、`/version`、
`/capabilities`、`worker-data` mapping 和真实 lease + evaluate readiness。任一步
失败由脚本恢复部署前的 6 个受管文件、两个 env 原字节及旧 5100 服务。

`-ProvisionDirectories` 与 `-ValidateOnly`、`-Rollback` 互斥，只读取未跟踪
container env 的 data/state 两个目录值。目标必须是
`C:\ProgramData\SoftwareDocGenerator` 的严格子目录，支持 `C:/` 与 `C:\`
写法；根、相对/drive-relative/POSIX/UNC、普通文件和 reparse point 均拒绝。
缺失目标只创建空目录并继承父 ACL，已有目录仅幂等验证，不清空、不迁移、
不改 ACL，也不触碰 env、服务、addon、Hermes 或 MATLAB 产物。`ValidateOnly`
...
的 WX11P 尚未验证 lease contract；启用九阶段前必须部署 v8 并取得
lease + evaluate readiness 证据。
