...
if (process.env.APP_ENV_FILE) {
  loadDotEnv(resolveRuntimePath(process.env.APP_ENV_FILE, ""), {
    canOverride(key) {
      return !externalEnvKeys.has(key);
    }
  });
...
function normalizeRuntimeRole(value = "") {
  const role = String(value || "all-in-one").trim().toLowerCase();
  return ["all-in-one", "platform", "hermes-agent"].includes(role) ? role : "all-in-one";
}

const runtimeRole = normalizeRuntimeRole(process.env.APP_RUNTIME_ROLE);
const appEnvironment = String(process.env.APP_ENV || process.env.APP_RUNTIME_ENV || process.env.NODE_ENV || "development")
  .trim()
  .toLowerCase();
const dataDir = resolveRuntimePath(process.env.APP_DATA_DIR, path.join(rootDir, "data"));
const skillsRootDir = resolveRuntimePath(process.env.APP_SKILLS_DIR, path.join(rootDir, "skills"));
...
const appConfig = {
  ...
  skillRootDir: skillsRootDir,
  activeSkillDir: path.join(skillsRootDir, "active"),
  skillBundleDir: path.join(skillsRootDir, "bundles"),
  ...
  skillDir: path.join(skillsRootDir, "active"),
  skillVersioning: {
...
function validateRuntimeRole(candidate = {}) {
  const productionLike = ["production", "prod"].includes(candidate.appEnvironment);
  if (!productionLike || candidate.runtimeRole !== "platform") {
    return;
  }

  if (candidate.hermes.transport === "cli") {
    throw new Error(
      "APP_RUNTIME_ROLE=platform in production must use HERMES_TRANSPORT=api and HERMES_BASE_URL to reach the Hermes Agent service."
    );
  }
  if (candidate.matlabMcp.transport === "stdio") {
    throw new Error(
      "APP_RUNTIME_ROLE=platform in production must use MATLAB_MCP_TRANSPORT=http and MATLAB_MCP_BASE_URL to reach the MATLAB worker."
    );
  }
}
