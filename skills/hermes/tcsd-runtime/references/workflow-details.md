...
The setup helper accepts explicit model init scripts as a second argument, and `TCSD_PROJECT_INIT_SCRIPTS` can provide a `;`/`,` separated list for unattended runs. It always merges those lists with recognized project bootstrap scripts auto-discovered in the workspace, so providing a model script cannot suppress required addon initialization. Recognized names include `init_Global.m`, `Global_*.m`, `startup.m`, `setup.m`, `initialize.m`, `init_*.m`, `*_init.m`, and similar setup/init patterns. Auto-discovered project/addon scripts run first, followed by environment-provided and explicit model scripts, with path-based de-duplication. The helper logs `TCSD_PROJECT_INIT_SCRIPTS_EXECUTED=...` for diagnosis while skipping generated/cache folders and its own helper scripts.
...
The TCSD action-step budget counts stimulus entries, not Simulink sample hits, solver evaluations, Unit Delay updates, counter increments, or elapsed hold cycles. A finite hold is one positive-duration action. Do not declare a stateful target unconstructible merely by dividing its required updates by the action-step budget; propose the bounded hold and let deterministic host simulation accept or reject it. A proposal that makes this conflation is a recoverable validation defect and must be retried from the structured validation report in a fresh Agent session.
...
For ramped outputs, do not write a single hold-style expectation. TCSD keeps or window-checks the sampled expected value over time, so a sampled ramp value becomes a wrong constant expectation. Prefer omitting that output from the step, or generate a separate dense 10 ms staircase only when the user explicitly wants ramp-shape checking. A scalar hold expectation is verified over the complete following interval, including its next-event endpoint; require simulation samples at both endpoints and at least two distinct timestamps before marking it stable.
...
Do not let expected-output stability rules reduce stimulus coverage. A Test may exist mainly to cover a decision outcome and may omit dynamic outputs from individual steps, but every delivered ordinary Test must retain at least one simulation-verified root-Outport `expValue(...)` in its Action. If no trustworthy oracle can be produced, fail the generation gate with the Test ID instead of delivering a case that MQTester will report as `None`.
