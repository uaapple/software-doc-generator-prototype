...
9. Write `model-index`, `hierarchy-manifest`, and `analysis-queue` to the paths supplied for those roles. The canonical top-level queue array field is exactly `items`; do not write `queueItems` or another alias. After writing `analysis-queue`, reread the persisted JSON and verify that `items` is an array containing the complete queue before writing a successful candidate. Do not perform the Stage 3 deep reads or write evidence claims in this stage.
...
- `analysis-queue`: the bounded ordered work items needed for Stage 3, stored in the canonical top-level `items` array. Every item has `parentDocumentUnit`; ordinary items use `analysisUnit`, while `scope=direct_outport` items use the exact `directOutport` field and may omit `analysisUnit`.
