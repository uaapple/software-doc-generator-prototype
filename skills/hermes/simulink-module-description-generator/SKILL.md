...
- Treat each module's externally visible outputs as the coverage boundary. Do not stop at the main calculation path or DocBlock summary; for every selected module, close the analysis at each Outport/exported output by tracing backward through the output cone.
...
9. For each module, write `功能描述` and `实现方式`. Keep `设计依据` blank unless the user explicitly requests populated design bases. In `实现方式`, cover every non-routing row in the output coverage ledger at least once, either by exact output name or by an explicit grouped role.
10. Run the output self-check from `references/writing-rules.md` before creating the final file. If the draft omits any non-routing ledger output, revise the module section before generating DOCX.
11. Create DOCX from the template, preserving styles and heading structure. Provide Markdown only when requested or as a review preview.
12. Validate that every implementation claim has traceable model evidence and that no required module is missing.
13. Clean MATLAB task state before finishing when safe: close loaded models opened for this task and avoid leaving temporary paths or variables that affect future tasks.
...
- Mention auxiliary state-holding outputs such as `*Rem*`, `*Old*`, `*Pre*`, `*Save*`, and `*EEW*` when they leave the module or affect final outputs. Describe when the value is held, updated, or restored; do not reduce these paths to anonymous Switch/Unit Delay plumbing.
