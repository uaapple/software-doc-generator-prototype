...
- Cover every externally meaningful module output at least once. This includes final outputs, exported signals, auxiliary outputs such as `*Rem*`, `*Rstr*`, `*Restore*`, `*Old*`, `*Pre*`, `*Last*`, `*Mem*`, `*Save*`, `*EEW*`, raw/final pairs, and mode restore outputs. Collapse pure duplicate routing, but do not omit a state-holding, output-shaping, or mode-restore output just because the main calculation output was already described.
