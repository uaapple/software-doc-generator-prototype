...
9. Before writing a successful candidate, self-check the union of shards against both `hierarchy-manifest` and `analysis-queue`: every queue item is represented by its own shard, every selected document unit has at least one shard, and every direct Outport appears in the corresponding shard's `outports`. A limitation does not replace direct-Outport evidence.
...
- `evidence-shards`: an index plus one persisted private shard artifact for every completed analysis-queue item. The union must retain the per-item evidence needed to aggregate each parent document unit; each `direct_outport` shard lists the queue item's exact `directOutport` in `outports`.
