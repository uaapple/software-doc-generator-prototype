...
- `../software-detail-runtime/scripts/satk_eval.py` only when the supplied execution environment uses that MCP bridge.
...
4. Start one task-owned MATLAB process/session for the job. For an unattended/front-end job, use a new task-owned process or MCP/SATK `new` session; attach only when the user explicitly supplied an active session or the platform provided one. Do not pre-create another MATLAB session for later stages.
5. In that same MATLAB session, run `setup_module_doc_support.m`, add the selected workspace project/addon files once, run the sanitized initialization scripts once, load model data as required, and call `load_system` for the target model once. If `satk_eval.py` is the supplied bridge, keep its default `SATK_MATLAB_SESSION_MODE=new` for the new task-owned session. Do not reopen, close, or repeatedly initialize the model. (`SDD-WF-002`)
...
- `matlab-session-lease`: identifies the live MATLAB session, its owning job, its task-owned lifecycle, and the model loaded once in that session so later stages can reuse it.
