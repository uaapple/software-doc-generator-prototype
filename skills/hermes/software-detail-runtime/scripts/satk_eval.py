...
def workspace_tool_candidates(tool_name: str) -> list[Path]:
    candidates: list[Path] = []
    for parent in [Path.cwd(), *Path.cwd().parents]:
        candidates.append(parent / "tools" / executable_name(tool_name))
    return candidates


def default_log_folder() -> str:
    if platform.system() == "Windows":
        return r"C:\Temp\matlab-mcp-core-server-module-doc"
    if platform.system() == "Darwin":
        return "/private/tmp/matlab-mcp-core-server-module-doc"
    return str(Path(tempfile.gettempdir()) / "matlab-mcp-core-server-module-doc")
...
DEFAULT_SERVER = (
    ...
    if os.environ.get("SATK_MCP_SERVER")
        ...
        [
            Path.home() / ".matlab" / "agentic-toolkits" / "bin" / executable_name("matlab-mcp-server"),
            Path.home() / ".matlab" / "agentic-toolkits" / "bin" / executable_name("matlab-mcp-core-server"),
            *workspace_tool_candidates("matlab-mcp-server"),
            *workspace_tool_candidates("matlab-mcp-core-server"),
        ]
    )
)
DEFAULT_EXTENSION = (
    ...
    commands = [
        ...
        [
            ...
            (
                ...
                "Where-Object { $_.Name -in @('matlab-mcp-server.exe','matlab-mcp-core-server.exe') } | "
...
def command_matches_task_mcp(command: str) -> bool:
    lower = command.lower()
    server_names = {
        executable_name("matlab-mcp-server").lower(),
        executable_name("matlab-mcp-core-server").lower(),
    }
    log_folder = str(LOG_FOLDER).lower()
    alt_log_folder = log_folder.replace("\\", "/")
    return any(server_name in lower for server_name in server_names) and (
        log_folder in lower or alt_log_folder in lower
    )
        ...
        send(
            proc,
            {
                ...
                "params": {
                    ...
                    "capabilities": {},
                    "clientInfo": {"name": "codex-simulink-module-doc-satk-eval", "version": "0.1"},
                },
            },
        )
