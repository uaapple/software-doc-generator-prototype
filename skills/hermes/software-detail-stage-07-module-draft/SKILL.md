...
2. Read the `document_unit` list carried by `boundary-projection` or `narrative-plan`, then confirm that every listed unit has at least one evidence shard plus a complete ledger, boundary projection, behavior groups, and narrative plan. Reject any direct boundary output marked missing or unsupported; do not draft it from port contracts, module names, or inferred behavior. Do not require or fetch a separate hierarchy-manifest input. Treat the whole-model phase as index-only and consume the per-subsystem evidence already produced upstream; never substitute a single scan or top-level port summary (`SDD-DEF-006`).
