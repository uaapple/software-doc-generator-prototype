...
## 当前阶段
阶段 9（主线验证） / 阶段 10（用户 Wiki 与使用说明） / 阶段 11（Windows 正式发布部署）
...
### 阶段 11：Windows 正式发布部署
- [x] 支持通过 `APP_DATA_DIR` 将正式工程、上传物、任务产物、反馈、工单与 `skills.sqlite` 放到代码目录之外
- [x] 支持通过 `APP_SKILLS_DIR` 将正式可变 skill active/bundles 目录放到代码目录之外
- [x] 新增 Mac 侧 `release/windows-prod` 分支限定的 zip 发布包脚本
- [x] 新增 Windows VM 一键部署脚本，覆盖停服务、解压、安装依赖、切换 current、启动服务、健康检查与失败回滚
- [x] 初版部署时从发布包初始化当前技能库到 `prod-skills`，后续部署不覆盖正式技能库
- [x] 新增 Windows 服务安装脚本，基于 WinSW 管理主服务与 Wiki 服务
- [x] 补充 Windows VM zip 部署文档与 README 入口
- [x] 补充自动化测试覆盖发布脚本、部署脚本和外置运行态配置
- **状态：** complete
...
| 正式发布采用 `release/windows-prod` + zip 包 + Windows VM 一键部署 | 保持开发分支并行，同时让正式环境只消费经过发布分支提升的代码版本 |
| 正式数据与代码发布包分离 | 避免部署新代码时覆盖正式工程、上传文件、技能库与生成产物 |
