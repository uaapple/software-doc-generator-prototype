...
function verifyWorkerGateway() {
  const script = [
    ...
    "if(!capabilities.ok)throw new Error('capabilities');",
    "const capabilityPayload=await capabilities.json();",
    "if(!capabilityPayload.operations?.includes('evaluate_matlab_code')||capabilityPayload.contracts?.leases!==true)throw new Error('capabilities');",
    "const suffix=`${Date.now()}-${Math.random().toString(16).slice(2)}`;",
    "const workspaceId=`deploy-ready-${suffix}`;",
    "const assetId='probe';",
    "const jobId=`deploy-ready-${suffix}`;",
    "const ownerJobId=`deploy-owner-${suffix}`;",
    "const leaseId=`deploy-lease-${suffix}`;",
    ...
    "response=await fetch(new URL(`/api/workspaces/${workspaceId}/leases/${leaseId}`,base),{method:'PUT',headers,body:JSON.stringify({ownerJobId})});",
    "if(!response.ok)throw new Error('lease-create');",
    "const lease=await response.json();",
    "if(lease.status!=='active'||lease.leaseId!==leaseId||lease.ownerJobId!==ownerJobId)throw new Error('lease-contract');",
    "leaseCreated=true;",
    ...
    "response=await fetch(new URL(`/api/jobs/${jobId}`,base),{method:'POST',headers:{...headers,'x-sdg-evaluate-token':evaluateToken,'x-sdg-gateway-caller':'tcsd-runtime'},body:JSON.stringify({workspaceId,operation:'evaluate_matlab_code',inputAssetId:assetId,timeoutMs:120000,leaseId,ownerJobId})});",
    ...
    "}finally{",
    "if(leaseCreated)await fetch(new URL(`/api/workspaces/${workspaceId}/leases/${leaseId}`,base),{method:'DELETE',headers,body:JSON.stringify({ownerJobId})}).catch(()=>{});",
    "await fetch(new URL(`/api/workspaces/${workspaceId}`,base),{method:'DELETE',headers}).catch(()=>{});",
    "}",
    "console.log('Worker authentication and MATLAB evaluate and lease readiness verified.');",
